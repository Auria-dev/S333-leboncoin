...
@section('content')
    ...
    <div class="reservation-container">
        <div class="reservation-form-column">
            ...
            <div style="padding: 15px; border-radius: var(--radius-sm); margin-bottom: 20px; font-weight: bold; 
            ...
            <form action="{{ url('/reservation/update/' . $reservation->idreservation) }}" method="POST"
                x-data="formManager({
                ...
                @submit.prevent="submitForm">
                    ...
                    <div class="field-group" x-data="inputField()">
                        ...
                        <div class="input-wrapper">
                            <input type="text" class="flatpickr-range" 
                                   ...
                                   :disabled="!editing" x-ref="input" @input="updateDate($el.value)">
                    ...
                    <div class="mt-md">
                        ...
                        @if($canEdit)
                            ...
                            <div class="picker-group" style="{{ $max_animaux <= 0 ? 'opacity:0.5; pointer-events:none;' : '' }} border-bottom: none;">
                                <div class="picker-label-container">
                        ...
                        @else
                            <div class="read-only-guests" style="display:flex; flex-direction:column; padding: 10px 0; gap:1rem;">
                                <p>{{ $reservation->nb_adultes }} Adulte(s)</p>
                                <p>{{ $reservation->nb_enfants }} Enfant(s)</p>
                                @if(($reservation->nb_bebes ?? 0) > 0) <p>{{ $reservation->nb_bebes }} Bébé(s)</p> @endif
                                @if(($reservation->nb_animaux ?? 0) > 0) <p>{{ $reservation->nb_animaux }} Animaux</p> @endif
                            </div>
                        @endif
                    </div>
                ...
                <div class="mt-md" style="border-top: 1px solid var(--border-default);">
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ...
                        @if($isRequester)
                            @if($reservation->statut_reservation == 'validée' && $now->format('Y-m-d') >= $reservation->date_debut_resa && $now->format('Y-m-d') <= $reservation->date_fin_resa)
                                <a href="{{ url('/reservation/declare/' . $reservation->idreservation) }}" class="other-btn" style="color: var(--danger); border-color: var(--danger);">Déclarer un incident</a>
                            @endif
                        @endif
                        ...
                        @if($isOwner)
                            ...
                            <div style="display:flex; justify-content: space-between; align-items: center; width: 100%;">                                
                                <form action="{{ url('/reservation/accept/' . $reservation->idreservation) }}" method="POST" style="width: calc(50% - 0.5rem); display:inline;">
                                    @csrf
                                    <button type="submit" class="submit-btn" style="width: 100%;">Accepter</button>
                                </form>
                                <form action="{{ url('/reservation/refuse/' . $reservation->idreservation) }}" method="POST" style="width: calc(50% - 0.5rem); display:inline;">
                                    @csrf
                                    <button type="submit" class="other-btn" style="width: 100%; color: var(--danger); border-color: var(--danger);">Refuser</button>
                                </form>
                            </div>
                            @endif
                        @endif
        ...
        <div style="position: sticky; top: 4.5rem;">
            <div class="reservation-summary-card mb-sm">
                ...
                <div class="price-row">
                    ...
                    <span>{{ $reservation->montant_total - $reservation->frais_services - $reservation->taxe_sejour }} €</span>
                </div>
                <div class="price-row">
            ...
            <div class="reservation-summary-card">
                ...
                <div class="host-info" style="margin-bottom: 15px;">
                    <img src="{{ asset($otherProfile->photo_profil ?? 'assets/default-avatar.png') }}" class="host-avatar" style="width: 50px; height: 50px;">
                    <div>
                        <div class="host-name font-bold">{{ $otherProfile->prenom_utilisateur }} {{ $otherProfile->nom_utilisateur }}</div>
                        <div class="text-muted text-sm">{{ $otherProfile->ville->nom_ville ?? '' }}</div>
                    </div>
                </div>
                <a href="{{ url('/proprio/' . $otherProfile->idutilisateur) }}" class="other-btn" style="width: 100%; display: block; text-align: center;">Voir le profil</a>
            </div>
        </div>
        </div>
    </div>
